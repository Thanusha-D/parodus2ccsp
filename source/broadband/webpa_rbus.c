...
static char* serverVal = NULL;
static char* tokenVal = NULL;
static char* DNSVal = NULL;
...
rbusError_t webpaDataSetHandler(rbusHandle_t handle, rbusProperty_t prop, rbusSetHandlerOptions_t* opts) {
	...
	(strncmp(paramName, WEBPA_SERVER_URL, maxParamLen) != 0) &&
	(strncmp(paramName, WEBPA_TOKEN_SERVER_URL, maxParamLen) != 0) &&
	(strncmp(paramName, WEBPA_DNS_TEXT_URL, maxParamLen) != 0)
	) {
    ...
    }else if(strncmp(paramName, WEBPA_SERVER_URL, maxParamLen) == 0) {
        WalError("ServerURL param is not writable\n");
        return RBUS_ERROR_ACCESS_NOT_ALLOWED;
    }else if(strncmp(paramName, WEBPA_TOKEN_SERVER_URL, maxParamLen) == 0) {
        WalError("TokenServerURL param is not writable\n");
        return RBUS_ERROR_ACCESS_NOT_ALLOWED;
    }else if(strncmp(paramName, WEBPA_DNS_TEXT_URL, maxParamLen) == 0) {
        WalError("DNSTextURL param is not writable\n");
        return RBUS_ERROR_ACCESS_NOT_ALLOWED;
    }
...
rbusError_t webpaDataGetHandler(rbusHandle_t handle, rbusProperty_t property, rbusGetHandlerOptions_t* opts) {
    ...
    }else if(strncmp(propertyName, WEBPA_SERVER_URL, maxParamLen) == 0) {
        rbusValue_t value;
        rbusValue_Init(&value);

        if(serverVal){
            rbusValue_SetString(value, serverVal);
        }
        else{
                retPsmGet = rbus_GetValueFromPSMDB( WEBPA_SERVER_URL, &serverVal );
                if (retPsmGet != RBUS_ERROR_SUCCESS){
                        WalError("psm_get failed ret %d for parameter %s and value %s\n", retPsmGet, propertyName, serverVal);
                        return retPsmGet;
                }
                else{
                        WalInfo("psm_get success ret %d for parameter %s and value %s\n", retPsmGet, propertyName, serverVal);
                        rbusValue_SetString(value, serverVal);
                        WalInfo("After ServerVal set to value\n");
                }
        }
	//WalInfo("ServerURL value fetched is %s\n", value);
        rbusProperty_SetValue(property, value);
        rbusValue_Release(value);

    }else if(strncmp(propertyName, WEBPA_TOKEN_SERVER_URL, maxParamLen) == 0) {
        rbusValue_t value;
        rbusValue_Init(&value);

        if(tokenVal){
            rbusValue_SetString(value, tokenVal);
        }
        else{
                retPsmGet = rbus_GetValueFromPSMDB( WEBPA_TOKEN_SERVER_URL, &tokenVal );
                if (retPsmGet != RBUS_ERROR_SUCCESS){
                        WalError("psm_get failed ret %d for parameter %s and value %s\n", retPsmGet, propertyName, tokenVal);
                        return retPsmGet;
                }
                else{
                        WalInfo("psm_get success ret %d for parameter %s and value %s\n", retPsmGet, propertyName, tokenVal);
                        rbusValue_SetString(value, tokenVal);
                        WalInfo("After TokenVal set to value\n");
                }
        }
	//WalInfo("TokenServerURL value fetched is %s\n", value);
        rbusProperty_SetValue(property, value);
        rbusValue_Release(value);

     }else if(strncmp(propertyName, WEBPA_DNS_TEXT_URL, maxParamLen) == 0) {
        rbusValue_t value;
        rbusValue_Init(&value);

        if(DNSVal){
            rbusValue_SetString(value, DNSVal);
        }
        else{
                retPsmGet = rbus_GetValueFromPSMDB( WEBPA_DNS_TEXT_URL, &DNSVal );
                if (retPsmGet != RBUS_ERROR_SUCCESS){
                        WalError("psm_get failed ret %d for parameter %s and value %s\n", retPsmGet, propertyName, DNSVal);
                        return retPsmGet;
                }
                else{
                        WalInfo("psm_get success ret %d for parameter %s and value %s\n", retPsmGet, propertyName, DNSVal);
                        rbusValue_SetString(value, DNSVal);
                        WalInfo("After DNSVal set to value\n");
                }
        }
	//WalInfo("DNSTextURL value fetched is %s\n", value);
        rbusProperty_SetValue(property, value);
        rbusValue_Release(value);
     }	
...
rbusError_t rbus_GetValueFromPSMDB( char* paramName, char** paramValue)
{
	char psmName[256] = {'\0'};
	...
	*paramValue = NULL;
        ...
        WalInfo("rbus_GetValueFromPSMDB paramName is %s\n", paramName);       	

	parameterNames[0] = (char*)paramName;

	sprintf(psmName, "%s%s", "eRT.", DEST_COMP_ID_PSM);
	WalInfo("rbus_GetValueFromPSMDB psmName is %s\n", psmName);
	...
	if((err = rbus_invokeRemoteMethod(psmName, METHOD_GETPARAMETERVALUES, request, 6000, &response)) != RTMESSAGE_BUS_SUCCESS)
		...
		if(1/*valSize*/)
		{
			...
			if(buff && (strcmp(paramName, buff) == 0))
			{
	...
	WalInfo("rbus_GetValueFromPSMDB End\n");
	return errorcode;
}
/**
...
rbusError_t rbus_GetValueFromDB( char* paramName, char** paramValue)
{
	...
	return rbus_GetValueFromPSMDB( recordName, paramValue );
}
...
WDMP_STATUS regWebpaDataModel()
{
	...
	rbusDataElement_t dataElements[NUM_WEBPA_ELEMENTS] = {
		...
		{WEBPA_SERVER_URL, RBUS_ELEMENT_TYPE_PROPERTY, {webpaDataGetHandler, webpaDataSetHandler, NULL, NULL, NULL, NULL}},
		{WEBPA_TOKEN_SERVER_URL, RBUS_ELEMENT_TYPE_PROPERTY, {webpaDataGetHandler, webpaDataSetHandler, NULL, NULL, NULL, NULL}},
		{WEBPA_DNS_TEXT_URL, RBUS_ELEMENT_TYPE_PROPERTY, {webpaDataGetHandler, webpaDataSetHandler, NULL, NULL, NULL, NULL}}
...
void getValues_rbus(const char *paramName[], const unsigned int paramCount, int index, money_trace_spans *timeSpan, param_t ***paramArr, int *retValCount, int *retStatus)
{
	...
	for(cnt1 = 0; cnt1 < paramCount; cnt1++)
	{
		...
		WalInfo("After IndexMpa_WEBPAtoCPE retIndex = %d\n", retIndex);
		if(retIndex == -1)
		{
